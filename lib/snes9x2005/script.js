...
var ac = null;
var noSound = true;  // PERFORMANCE: audio disabled — saves ~30% CPU
...
// Default gamepad mapping for standard controllers (Xbox/PlayStation)
// Bit mapping: 4=R, 5=L, 6=X, 7=A, 8=Right, 9=Left, 10=Down, 11=Up, 12=Start, 13=Select, 14=Y, 15=B
// Gamepad standard: 0=A/Cross, 1=B/Circle, 2=X/Square, 3=Y/Triangle, 4=LB, 5=RB, 8=Select, 9=Start, 12=Up, 13=Down, 14=Left, 15=Right
g.gamepadconnectedHandler = function() {
    var idx = g.curGamepadIndex;
    if (!g.buttonRemappers[idx]) return;
    // Only set defaults if no saved settings
    var hasSettings = false;
    for (var i = 0; i < g.buttonRemappers[idx].length; i++) {
        if (g.buttonRemappers[idx][i] >= 0) { hasSettings = true; break; }
    }
    if (hasSettings) return;
    // Map: gamepad button index -> SNES bit
    var map = {
        0: 7,   // A/Cross -> SNES A
        1: 15,  // B/Circle -> SNES B
        2: 14,  // X/Square -> SNES Y
        3: 6,   // Y/Triangle -> SNES X
        4: 5,   // LB -> SNES L
        5: 4,   // RB -> SNES R
        8: 13,  // Back/Share -> SNES Select
        9: 12,  // Start/Options -> SNES Start
        12: 11, // DPad Up -> SNES Up
        13: 10, // DPad Down -> SNES Down
        14: 9,  // DPad Left -> SNES Left
        15: 8   // DPad Right -> SNES Right
    };
    for (var btn in map) {
        var b = parseInt(btn);
        if (b < g.buttonRemappers[idx].length) {
            g.buttonRemappers[idx][b] = map[btn];
        }
    }
    // Also map analog stick via axes
    if (g.axesRemappers[idx] && g.axesRemappers[idx].length >= 4) {
        g.axesRemappers[idx][0] = 8;  // Axis 0+ -> Right
        g.axesRemappers[idx][1] = 9;  // Axis 0- -> Left
        g.axesRemappers[idx][2] = 10; // Axis 1+ -> Down
        g.axesRemappers[idx][3] = 11; // Axis 1- -> Up
    }
};
...
Module.onRuntimeInitialized = function() {
  isModuleInited = true;
  // Auto-load ROM from URL param ?rom=path
  var params = new URLSearchParams(window.location.search);
  var romUrl = params.get('rom');
  if (romUrl) {
    // Audio disabled for performance (noSound=true set at top)
    // Set fullscreen mode and hide menu so run1fr() renders
    isFullScreen = true;
    isMenuDisplay = false;
    fetchRomFromUrl(romUrl);
  }
}
...
// Pre-allocate downscale buffer (256x224 = 229KB vs 512x448 = 917KB)
var ds_imgData = null;
var ds_ctx = null;
var useDownscale = true;
...
function run1fr(){
  ...
  if(useDownscale){
    // PERFORMANCE: Downscale 512x448 → 256x224 (copy every other pixel, every other row)
    // This is 4x less data to copy and push to canvas
    if(!ds_ctx){
      var c = isFullScreen ? fso : el("output");
      c.width = 256;
      c.height = 224;
      ds_ctx = c.getContext("2d");
      ds_imgData = ds_ctx.createImageData(256, 224);
    }
    var src = new Uint32Array(Module.HEAP8.buffer, frameBufferPtr, 512 * 448);
    var dst = new Uint32Array(ds_imgData.data.buffer);
    for(var y = 0; y < 224; y++){
      var srcRow = y * 2 * 512;
      var dstRow = y * 256;
      for(var x = 0; x < 256; x++){
    ...
    ds_ctx.putImageData(ds_imgData, 0, 0);
  }else{
    var frameBufferRawData = new Uint8ClampedArray(Module.HEAP8.buffer, frameBufferPtr, 512 * 448 * 4);
    if(isFullScreen){
