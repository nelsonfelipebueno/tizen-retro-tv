...
function canvasSetup(can){
  ...
  var canImgData = canCtx.createImageData(512, 448);
  dest.push(canCtx);
...
// PERFORMANCE: offscreen canvas for putImageData, then drawImage to visible canvas (GPU-accelerated)
var frameCount = 0;
var offCanvas = document.createElement('canvas');
offCanvas.width = 512;
offCanvas.height = 448;
var offCtx = offCanvas.getContext('2d');
var offImgData = offCtx.createImageData(512, 448);
...
function run1fr(){
  ...
  Module._mainLoop();
  // Frameskip: skip render 1 of every 4 frames (~45fps visual, 60fps logic)
  frameCount++;
  ...
  offImgData.data.set(frameBufferRawData);
  offCtx.putImageData(offImgData, 0, 0);
  if(isFullScreen){
    fs_ctx.drawImage(offCanvas, 0, 0);
  }else{
